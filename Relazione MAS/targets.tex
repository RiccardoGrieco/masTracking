...
I targets hanno un comportamento molto semplice: appaiono in maniera casuale nella mappa e si muovono fra le diverse stanze. Lo \textit{spawn} dei target è gestito dall'omonima classe Target, la quale attende un periodo di tempo prefissato prima di far apparire un nuovo target. Nella classe Target viene inoltre specificato il numero massimo di target permessi. Lo spawn è gestito in modo da far apparire i target solo in punti liberi della mappa. Il movimento dei target prevede due fasi distinte:
\begin{itemize}

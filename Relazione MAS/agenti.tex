...
Ogni agente viene inizializzato per mezzo di una serie di \textit{percept}, i quali vengono estratti da un file JSON di configurazione. Gli agenti telecamera hanno posizione fissa e una porzione di spazio a loro visibile. Tale area di visione può essere sovrapposta con quella di altri agenti, ma nessuno di questi ha informazioni sui parametri degli altri. Fra i percept iniziali troviamo anche quello riferito al numero di vicini, cioè il numero di agenti con cui un particolare agente può comunicare. Per comodità di utilizzo del framework di simulazione, questi percept iniziali vengono trasformati in \textit{mental notes} dell'agente.
...
\'E stata prevista una sola regola per gli agenti: amInterested(X,Y), la quale è vera se l'agente è interessato ad un target in posizione (X,Y).\\ \\
